...
public class LoanAnalyticsService {
    ...
    @Cacheable(value = "loanPortfolioAnalytics", key = "'portfolio_v2'")
    public PortfolioAnalyticsDTO getPortfolioAnalytics() {
        log.info("Computing loan portfolio analytics (cache miss)");

        // 1. Top Level KPIs
        BigDecimal totalAum = loanRepository.getTotalAum();
        if (totalAum == null) totalAum = BigDecimal.ZERO;

        Long activeLoansCount = loanRepository.countActiveLoans();
        if (activeLoansCount == null) activeLoansCount = 0L;

        BigDecimal totalNpaAmount = loanRepository.getTotalNpaAmount();
        if (totalNpaAmount == null) totalNpaAmount = BigDecimal.ZERO;

        BigDecimal totalParAmount = loanRepository.getTotalParAmount();
        if (totalParAmount == null) totalParAmount = BigDecimal.ZERO;
        ...
        // Placeholder for Collection Efficiency (Requires Transaction Data Analysis, setting simplified mock for now if needed, or derived)
        // For now, we set it to a static realistic value or derived from DPD stats if possible.
        ...
        List<Object[]> riskRaw = loanRepository.getRiskBucketDistribution();
        ...
        // 3. State Performance
        List<Object[]> stateRaw = loanRepository.getStateWisePerformance();
        ...
        List<Object[]> schemeRaw = loanRepository.getSchemeWisePerformance();
        ...
        List<Object[]> providerRaw = loanRepository.getProviderPerformance();
        ...
        List<Object[]> genderRaw = loanRepository.getBorrowerGenderDistribution();
        ...
        // 7. Demographics - Age
        List<Object[]> ageRaw = loanRepository.getBorrowerAgeDistribution();
