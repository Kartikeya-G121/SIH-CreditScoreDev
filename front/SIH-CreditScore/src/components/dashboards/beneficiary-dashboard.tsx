...
>>>>>>> 61964b08988c0d29c26197d75bc4030f035c9d24
          ...
          <Card className="border-0 shadow-lg">
            <CardHeader className="pb-6">
              <CardTitle className="flex items-center gap-2 text-xl">
                ...
                {safeT(t, 'repayment_schedule', 'Repayment Schedule')}
              </CardTitle>
              <CardDescription className="text-base">{safeT(t, 'upcoming_payments', 'Your upcoming and past payments')}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="rounded-lg border bg-card/50">
                <Table>
                  ...
                  <TableBody>
                    {formattedRepayments.map(payment => (
          ...
          <Card className="border-0 shadow-lg">
            <CardHeader className="pb-6">
              <CardTitle className="flex items-center gap-2 text-xl">
                ...
                {safeT(t, 'repayment_trends', 'Repayment Trends')}
              </CardTitle>
              <CardDescription className="text-base">{safeT(t, 'payment_performance', 'Your payment performance over time')}</CardDescription>
            </CardHeader>
            <CardContent className="pt-2">
              <ChartContainer config={repaymentChartConfig} className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={repaymentTrends} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        <TabsContent value="profile" className="space-y-8 mt-0 p-1">
          <Card>
            <CardHeader>
              <CardTitle>{safeT(t, 'profile_information', 'Profile Information')}</CardTitle>
              <CardDescription>{safeT(t, 'manage_details', 'Manage details')}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <ProfileBlock user={user ?? undefined} t={t} />
              <Button
                variant="outline"
                onClick={() => setShowEditDialog(true)}
              >
                {safeT(t, 'edit_profile', 'Edit Profile')}
              </Button>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{safeT(t, 'saved_bills', 'Saved Bills')}</CardTitle>
              <CardDescription>{safeT(t, 'saved_bills_desc', '')}</CardDescription>
            </CardHeader>
            <CardContent>
              {savedBills.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">{safeT(t, 'no_bills_saved', "You haven't saved any bills yet. Upload one in the 'Bill Upload' tab.")}</p>
              ) : (
