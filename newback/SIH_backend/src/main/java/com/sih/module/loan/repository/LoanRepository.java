...
public interface LoanRepository extends JpaRepository<Loan, Long>, JpaSpecificationExecutor<Loan> {
        ...
        @Query("SELECT SUM(l.outstandingPrincipal) FROM Loan l " +
                        "JOIN l.application la JOIN la.scheme s " +
                        "WHERE l.loanStatus IN ('ACTIVE', 'OVERDUE', 'DEFAULTED') " +
                        "AND (:providerName IS NULL OR s.providerName = :providerName)")
        BigDecimal getTotalAum(@Param("providerName") String providerName);

        @Query("SELECT COUNT(l) FROM Loan l " +
                        "JOIN l.application la JOIN la.scheme s " +
                        "WHERE l.loanStatus = 'ACTIVE' " +
                        "AND (:providerName IS NULL OR s.providerName = :providerName)")
        Long countActiveLoans(@Param("providerName") String providerName);

        @Query("SELECT SUM(l.outstandingPrincipal) FROM Loan l " +
                        "JOIN l.application la JOIN la.scheme s " +
                        "WHERE l.riskBucket = 'NPA' " +
                        "AND (:providerName IS NULL OR s.providerName = :providerName)")
        BigDecimal getTotalNpaAmount(@Param("providerName") String providerName);

        @Query("SELECT SUM(l.outstandingPrincipal) FROM Loan l " +
                        "JOIN l.application la JOIN la.scheme s " +
                        "WHERE l.dpd > 30 AND l.loanStatus IN ('ACTIVE', 'OVERDUE', 'DEFAULTED') " +
                        "AND (:providerName IS NULL OR s.providerName = :providerName)")
        BigDecimal getTotalParAmount(@Param("providerName") String providerName);
        ...
        List<Object[]> getRiskBucketDistribution(@Param("providerName") String providerName);

        // State-wise Performance
        // Returns: [state, count, totalAum, npaAmount]
        // Note: npaAmount calculation requires conditional sum, using logic: SUM(CASE
        // WHEN riskBucket='NPA' THEN outstandingPrincipal ELSE 0 END)
        @Query(value = "SELECT bp.state, " +
                        ...
                        "AND (:providerName IS NULL OR s.provider_name = :providerName) " +
                        "GROUP BY bp.state", nativeQuery = true)
        List<Object[]> getStateWisePerformance(@Param("providerName") String providerName);
        ...
        List<Object[]> getSchemeWisePerformance(@Param("providerName") String providerName);
        ...
        List<Object[]> getProviderPerformance(@Param("providerName") String providerName);
        ...
        List<Object[]> getBorrowerGenderDistribution(@Param("providerName") String providerName);
        ...
        List<Object[]> getBorrowerAgeDistribution(@Param("providerName") String providerName);
}
