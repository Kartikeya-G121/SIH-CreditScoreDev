...
public class ApplicationAnalyticsService {
        ...
        @Cacheable(value = "applicationAnalytics", key = "#providerName != null ? #providerName : 'global'")
        public ApplicationAnalyticsResponse getApplicationAnalytics(String providerName) {
                log.info("Computing application analytics (cache miss) for provider: {}",
                                providerName != null ? providerName : "ALL");

                // Overall statistics
                // Overall statistics
                ApplicationStatsDTO overallStats = ApplicationStatsDTO.builder()
                                .totalApplications(applicationRepository.countAll(providerName))
                                .draftCount(applicationRepository.countByStatus("DRAFT", providerName))
                                .submittedCount(applicationRepository.countByStatus("SUBMITTED", providerName))
                                .scoringCount(applicationRepository.countByStatus("SCORING", providerName))
                                .approvedCount(applicationRepository.countByStatus("APPROVED", providerName))
                                .rejectedCount(applicationRepository.countByStatus("REJECTED", providerName))
                                .sanctionedCount(applicationRepository.countByStatus("SANCTIONED", providerName))
                                .withdrawnCount(applicationRepository.countByStatus("WITHDRAWN", providerName))
                                .build();

                // State-wise statistics
                // State-wise statistics
                List<Object[]> stateStatsRaw = applicationRepository.getStateWiseStatistics(providerName);
        ...
        public ApplicationSearchResponse advancedSearch(ApplicationSearchRequest request) {
                log.info("Advanced search request: searchText='{}', status='{}', state='{}', schemeId={}",
                                request.getSearchText(), request.getStatus(), request.getState(),
                                request.getSchemeId());

                // Default pagination - use unsorted pageable to avoid column name conflicts in
                // native query
